function [T, p, h, m, w, eta, Q, PP, s] = doubleLift_model_NH3H2O(T, p, h, m, eta, Q, HX, s)
...
p.mid = sqrt(p.evap*p.cond); % Venegas
% ----------------------------------------------------------------------- %
%% Refrigerant line
% Desorber
...
cp.sol_pump_out = refpropm('C','T',T.sol_pump_out,'P',p.mid/1000,'AMMONIA','WATER',[w.NH3_rich (1-w.NH3_rich)]);
% High pressure cycle
% Absorber
w.NH3_richI = NH3inSolution_Calc_X_PT(p.mid/1000,T.sol_abs_outI);
if (w.NH3_richI < 0)
    error("w_NH3_richI < 0")
end
...
try
    ...
    % Assuming constant volume
    cv.sol_abs_outI = refpropm('O','T',T.sol_abs_outI,'Q',0,'AMMONIA','WATER',[w.NH3_richI (1-w.NH3_richI)]);
    T.sol_pump_outI = T.sol_abs_outI + (h.sol_pump_outI-h.sol_abs_outI)/cv.sol_abs_outI;
end
cp.sol_pump_outI = refpropm('C','T',T.sol_pump_outI,'P',p.cond/1000,'AMMONIA','WATER',[w.NH3_richI (1-w.NH3_richI)]);
%-------------------------------------------------------------------------%
%% Poor solution (Low ref. concentration)
% High pressure
% Desorber
w.NH3_poorI = NH3inSolution_Calc_X_PT(p.cond/1000,T.sol_des_outI);
...
if(w.NH3_poorI>0)
    ...
    h.sol_des_outI = refpropm('H','T',T.sol_des_outI,'P',p.cond,'WATER');
end
% SHEX
...
w.NH3_poor = NH3inSolution_Calc_X_PT(p.mid/1000,T.sol_des_out);
