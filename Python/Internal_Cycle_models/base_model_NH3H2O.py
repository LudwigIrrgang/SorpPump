...
def base_model_NH3H2O(T, p, h, m, eta, Q, HX, s):
    ...
    w.NH3_rich = RP.REFPROPdll(hFld="AMMONIA;WATER", hIn="PT", hOut= "XMASS", iUnits=RP_Unit,iMass=1, iFlag=0, a=p.evap*10**(-3), b=T.sol_abs_out, z=[w.NH3_rich, (1-w.NH3_rich)]).Output[0]
    ...
    h.sol_pump_isentropic = RP.REFPROPdll(hFld="AMMONIA;WATER", hIn="PS", hOut= "H", iUnits=RP_Unit,iMass=1, iFlag=0, a=p.cond*10**(-3), b=s.sol_pump_out*10**(-3), z=[w.NH3_rich, (1-w.NH3_rich)]).Output[0]*1000
    h.sol_pump_out = h.sol_abs_out - (h.sol_abs_out-h.sol_pump_isentropic)/eta.pump
    T.sol_pump_out = RP.REFPROPdll(hFld="AMMONIA;WATER", hIn="PH", hOut= "T", iUnits=RP_Unit, iMass=1, iFlag=0, a=p.cond*10**(-3), b=h.sol_pump_out*10**(-3), z=[w.NH3_rich, (1-w.NH3_rich)]).Output[0]
    cp.sol_pump_out = RP.REFPROPdll(hFld="AMMONIA;WATER", hIn="PH", hOut= "CP", iUnits=RP_Unit, iMass=1, iFlag=0, a=p.cond*10**(-3), b=h.sol_pump_out*10**(-3), z=[w.NH3_rich, (1-w.NH3_rich)]).Output[0]*1000
    #-------------------------------------------------------------------------#
    ## Poor solution (Low ref. concentration)
    # Desorber
    w.NH3_poor = NH3H2O.NH3inSolution_Calc_X_PT(p.cond,T.sol_des_out)
    w.NH3_poor = RP.REFPROPdll(hFld="AMMONIA;WATER", hIn="PT", hOut= "XMASS", iUnits=RP_Unit,iMass=1, iFlag=0, a=p.cond*10**(-3), b=T.sol_des_out, z=[w.NH3_poor, (1-w.NH3_poor)]).Output[0]
    if(w.NH3_poor>0):
    ...
    h.sol_valve_in = RP.REFPROPdll(hFld="AMMONIA;WATER", hIn="PT", hOut= "H", iUnits=RP_Unit,iMass=1, iFlag=0, a=p.cond*10**(-3), b=T.sol_valve_in, z=[w.NH3_poor, (1-w.NH3_poor)]).Output[0]*1000
    #-------------------------------------------------------------------------#
    ...
    try:
        T.sol_des_in = RP.REFPROPdll(hFld="AMMONIA;WATER", hIn="PH", hOut= "T", iUnits=RP_Unit,iMass=1, iFlag=0, a=p.cond*10**(-3), b=h.sol_des_in*10**(-3), z=[w.NH3_rich, (1-w.NH3_rich)]).Output[0]
    except:
    ...
    try: 
        T.sol_abs_in = RP.REFPROPdll(hFld="AMMONIA;WATER", hIn="PH", hOut= "T", iUnits=RP_Unit,iMass=1, iFlag=0, a=p.evap*10**(-3), b=h.sol_abs_in*10**(-3), z=[w.NH3_poor, (1-w.NH3_poor)]).Output[0]
    except:
